...
import { trackPrescriberClick } from "@/lib/tracking.functions";

export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      { title: "Contact & Practice Onboarding — Wellness Tech Bio Distribution" },
      { name: "description", content: "Request access to the Wellness Tech Bio Distribution catalog. Practice verification, account setup, and clinical support for licensed physicians." },
      { property: "og:title", content: "Contact — Wellness Tech Bio Distribution" },
      { property: "og:description", content: "Onboard your practice or request a quote." },
    ],
  }),
  component: Contact,
});
...
const steps = [
  { n: "01", title: "Practice Verification", desc: "Submit your medical license, DEA registration, and practice documentation. All ordering accounts require active physician credentials." },
  { n: "02", title: "Account Setup", desc: "We onboard your practice with a dedicated account representative, pricing schedule, and access to our full catalog and ordering portal." },
  { n: "03", title: "Protocol Selection", desc: "Choose from 100+ compounded Rx products across GLP-1, peptides, hormone, and longevity categories." },
  { n: "04", title: "Order & Dispense", desc: "Place orders directly with our 503A/503B partner pharmacies — patient-specific or facility-stock." },
  { n: "05", title: "Clinical Support", desc: "Our clinical team assists with dosing protocols, product selection, patient education, and compliance guidance." },
];

function Contact() {
  const [callStatus, setCallStatus] = useState<"idle" | "ok">("idle");
  const [callErrs, setCallErrs] = useState<Record<string, string>>({});
  const trackClick = useServerFn(trackPrescriberClick);
    ...
    <SiteLayout>
      <section className="bg-primary text-primary-foreground">
        <div className="container-x py-24 md:py-32">
          ...
          <h1 className="mt-3 max-w-3xl font-display text-5xl font-semibold leading-tight md:text-7xl">Onboard your practice in five steps.</h1>
          <p className="mt-6 max-w-2xl text-primary-foreground/90">Verified credentials unlock pricing, COAs, and ordering. Our clinical team responds within one business day.</p>
        </div>
      </section>

      <section className="container-x py-24">
        <div className="mb-14 overflow-hidden rounded-sm border border-primary/10">
          <img src={peopleConsult} alt="Physician and practice owner in onboarding consultation" width={1280} height={896} loading="lazy" className="h-64 w-full object-cover md:h-96" />
        </div>
      ...
      <section className="container-x grid gap-16 pb-24 md:grid-cols-12">
        <div className="md:col-span-4 space-y-8">
          <ContactLine icon={Mail} label="Email" value="Admin@thewellnesstech.com" href="mailto:Admin@thewellnesstech.com" />
          <ContactLine icon={Phone} label="Practitioner line" value="877-847-6423" href="tel:8778476423" />
          <ContactLine icon={MapPin} label="Coverage" value="Nationwide · 503A & 503B network" />
          <div className="rounded-sm border border-primary/10 bg-primary/5 p-6">
            ...
            <p className="mt-2 text-sm text-primary/80">Mon–Fri, 8a–7p ET<br />Clinical support 24/7 for active accounts.</p>
          </div>
        </div>
        <div className="md:col-span-8 flex items-center justify-center rounded-sm border border-primary/10 bg-card p-8 md:p-10">
          <div className="text-center">
            ...
            <a
              ...
              onClick={async (e) => {
                e.preventDefault();
                await trackClick({ data: { source: "contact_page_register_button" } });
                window.location.href = PRESCRIBER_URL;
              }}
        ...
        <div className="rounded-sm border border-primary/10 bg-card p-8 md:p-10">
          ...
          <form onSubmit={onCallSubmit} className="mt-8 grid gap-5 md:grid-cols-2">
            ...
            <label className="block">
              <span className="text-xs font-semibold uppercase tracking-widest text-primary/70">Best time to call</span>
              <select name="bestTime" defaultValue="" className="mt-2 w-full rounded-sm border border-input bg-background px-3 py-2.5 text-sm outline-none focus:border-accent">
              ...
              {callErrs.bestTime && <span className="mt-1 block text-xs text-destructive">{callErrs.bestTime}</span>}
            </label>
            ...
            {callStatus === "ok" && <p className="md:col-span-2 text-sm text-accent">Thanks — a customer service representative will call you at the requested time.</p>}
            <div className="md:col-span-2">
              <button type="submit" className="inline-flex items-center justify-center gap-2 rounded-sm bg-accent px-6 py-3.5 text-sm font-semibold uppercase tracking-wider text-accent-foreground transition hover:bg-gold-soft">
...
function ContactLine({ icon: Icon, label, value, href }: { icon: React.ComponentType<{ className?: string }>; label: string; value: string; href?: string }) {
  return (
    <div>
      ...
      {href ? <a href={href} className="mt-1 block font-display text-lg text-primary hover:text-accent">{value}</a> : <div className="mt-1 font-display text-lg text-primary">{value}</div>}
    </div>
  );
}
