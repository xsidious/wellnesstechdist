...
import { z } from "zod";
import { ArrowUpRight, Package, Syringe, Stethoscope, Sparkles, ShieldCheck, Truck } from "lucide-react";

export const Route = createFileRoute("/supplies")({
  head: () => ({
    meta: [
      { title: "Aesthetic & Medical Supplies — Wellness Tech Bio Distribution" },
      { name: "description", content: "Distribution of aesthetic and medical supplies for clinics, med spas and physician practices — devices, consumables, injectables ancillaries and clinic essentials." },
      { property: "og:title", content: "Aesthetic & Medical Supplies — Wellness Tech Bio Distribution" },
      { property: "og:description", content: "Wholesale aesthetic and medical supply distribution for licensed practices." },
    ],
  }),
  component: Supplies,
});
    ...
    <SiteLayout>
      <section className="border-b border-primary/10 bg-primary/5">
        <div className="container-x py-20 md:py-28">
          <span className="text-xs font-semibold uppercase tracking-widest text-accent">Distribution</span>
          ...
          <p className="mt-6 max-w-2xl text-muted-foreground">
            One source for the supplies that keep your practice running — from injectables ancillaries and aesthetic device consumables to clinical essentials and PPE. Wholesale pricing for verified medical and aesthetic practices, with custom sourcing available.
          </p>
        </div>
      </section>
      ...
      {status === "ok" && (
        <div className="border-b border-accent/30 bg-accent/10">
          <div className="container-x py-4 text-sm text-primary">
            Thank you — your supply inquiry was received. A distribution rep will reach out shortly with availability and pricing.
          </div>
        </div>
      )}
      ...
      <section id="inquiry" className="border-t border-primary/10 bg-primary/5">
        <div className="container-x grid gap-10 py-16 md:grid-cols-12 md:py-20">
          <div className="md:col-span-5">
            <span className="text-xs font-semibold uppercase tracking-widest text-accent">Supply Inquiry</span>
            <h2 className="mt-2 font-display text-3xl font-semibold text-primary md:text-4xl">
              Tell us what you need.
            </h2>
            <p className="mt-3 text-sm text-muted-foreground">
              Describe the supplies, brands, sizes or volumes you're sourcing. We respond within one business day with availability, pricing and lead time. New accounts may be asked to verify practice credentials before first shipment.
            </p>
          </div>
          <form onSubmit={onSubmit} className="md:col-span-7 rounded-sm border border-primary/10 bg-card p-6 md:p-8">
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <label className="block">
                <span className="text-xs font-semibold uppercase tracking-widest text-primary/70">Category</span>
                <select name="category" defaultValue="" className="mt-2 w-full rounded-sm border border-input bg-background px-3 py-2.5 text-sm outline-none focus:border-accent">
                ...
                {errs.category && <span className="mt-1 block text-xs text-destructive">{errs.category}</span>}
              </label>
              <label className="block md:col-span-2">
                <span className="text-xs font-semibold uppercase tracking-widest text-primary/70">Estimated volume / frequency</span>
                <select name="volume" defaultValue="" className="mt-2 w-full rounded-sm border border-input bg-background px-3 py-2.5 text-sm outline-none focus:border-accent">
                ...
                {errs.volume && <span className="mt-1 block text-xs text-destructive">{errs.volume}</span>}
              </label>
              <div className="md:col-span-2">
                <Field name="details" label="Items, brands, sizes, quantities" textarea rows={5} error={errs.details} />
              </div>
              <div className="md:col-span-2">
      ...
      <p className="container-x py-8 text-center text-xs text-muted-foreground">
        Distribution available to licensed medical and aesthetic practices only. Brand availability and pricing subject to change without notice.
      </p>
    </SiteLayout>
  );
}
...
function Field({
  ...
  return (
    <label className="block">
      ...
      {textarea ? (
        <textarea name={name} rows={rows} maxLength={1500}
