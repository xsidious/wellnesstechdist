    ...
    <div className="mx-auto max-w-2xl">
      <div className="rounded-sm border border-primary/10 bg-card p-8 md:p-10">
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          The Compounded Therapies catalog and product list are restricted. Complete the short intake form below to unlock access on this device.
        </p>
        ...
        <form onSubmit={onSubmit} className="mt-6 grid gap-5">
          ...
          <label className="flex items-start gap-2 text-xs text-primary/80">
            ...
            <span>I confirm the information above is accurate and I will use the catalog and product list in compliance with applicable state and federal regulations.</span>
          </label>
          {errs.agree && <span className="text-xs text-destructive">{errs.agree}</span>}

          <button type="submit" className="inline-flex items-center justify-center gap-2 rounded-sm bg-primary px-6 py-3.5 text-sm font-semibold uppercase tracking-wider text-primary-foreground transition hover:bg-primary/90">
        ...
        <div className="mt-6 border-t border-primary/10 pt-6 text-center text-xs text-muted-foreground">
          ...
          or{" "}
          <a href="/affiliates" className="text-accent hover:underline">Become an Affiliate</a>.
        </div>
      </div>
    </div>
  );
